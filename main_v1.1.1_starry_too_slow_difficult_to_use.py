...
with st.sidebar:
    ...
    if (db_mode == 'teMatDb'):
        ...
        prefix = "20211124_rawdata"
        file_tematdb_metadata_csv = "_Starrydata2_20211124_rawdata_meta_20230417_235615.xlsx"

        
        df_db_meta0 = pd.read_excel("./"+file_tematdb_metadata_csv, sheet_name='20211124_rawdata', )
        df_db_meta = df_db_meta0
        df_db_meta.index = list(df_db_meta.sampleid.copy())
        
        # df_db_csv = pd.read_csv("./data_csv/"+file_tematdb_db_csv)        
        df_alpha0 = pd.read_csv("./data_csv/"+"20211124_rawdata_alpha.csv")
        df_rho0   = pd.read_csv("./data_csv/"+"20211124_rawdata_rho.csv")
        df_kappa0 = pd.read_csv("./data_csv/"+"20211124_rawdata_kappa.csv")
        df_ZT0    = pd.read_csv("./data_csv/"+"20211124_rawdata_zT.csv")
        
        df_db_csv = pd.concat( [df_alpha0, df_rho0, df_kappa0, df_ZT0], ignore_index=True)
        
        df_db_extended_csv = pd.read_csv("./data_csv/"+"Starrydata2_20211124_rawdata_extendedTEPset.csv" )
        df_db_extended_csv = df_db_extended_csv[ df_db_extended_csv.is_Temp_in_autoTcTh_range ]
        df_db_extended_csv = df_db_extended_csv[ df_db_extended_csv.is_Temp_in_ZT_author_declared ] 
        
        file_tematdb_error_csv = "Starry_error.csv"
        df_db_error0 = pd.read_csv("./data_error_analysis/"+file_tematdb_error_csv)
        # err_cols = ['sampleid','JOURNAL', 'YEAR', 
        #        'Corresponding_author_main', 'Corresponding_author_institute',
        #        'Corresponding_author_email', 'figure_number_of_targetZT',
        ...
        # df_db_error0['Linf_over_avgZT']  = df_db_error0.Linf / df_db_error0.avgZT_TEP_on_Ts_TEP
        # df_db_error0['Linf_over_peakZT'] = df_db_error0.Linf / df_db_error0.peakZT_TEP_on_Ts_TEP
        df_db_error = pd.merge( df_db_error0, df_db_meta, on='sampleid', how='left')
        df_db_error.index = list(df_db_error.sampleid.copy())        
        ...
        df_alpha_sampleid = df_alpha0[ df_alpha0.sampleid == sampleid]
        df_rho_sampleid   = df_rho0[   df_rho0.sampleid == sampleid]
        df_kappa_sampleid = df_kappa0[ df_kappa0.sampleid == sampleid]
        df_ZT_sampleid    = df_ZT0[    df_ZT0.sampleid == sampleid]
        
        interp_opt = {MatProp.OPT_INTERP:MatProp.INTERP_LINEAR,\
                      MatProp.OPT_EXTEND_LEFT_TO:1,          # ok to 0 Kelvin
                      MatProp.OPT_EXTEND_RIGHT_BY:2000}        # ok to +50 Kelvin from the raw data
            
        def get_mat_tep(sampleid, df_alpha_sampleid, df_rho_sampleid, df_kappa_sampleid, df_ZT_sampleid, mat_name):
            mat = False
            TF_mat_complete = False
            
            
            try:
                mat = TEProp_df.load_from_df(df_alpha_sampleid, 
                                             df_rho_sampleid, 
                                             df_kappa_sampleid, 
                                             df_ZT_sampleid, mat_name=mat_name)
        ...
        mat_name = "Starrydata2_{}_{}".format(prefix,sampleid)
        TF_mat_complete, mat = get_mat_tep(sampleid, 
                                           df_alpha_sampleid, 
                                           df_rho_sampleid, 
                                           df_kappa_sampleid, 
                                           df_ZT_sampleid, mat_name)
...
with tab1:  
    ...
    if (db_mode == 'Starrydata2'):
        colnames = ['sampleid', 'sid', 'DOI', 'year', 'published',
                    'db', 'db_version',
                    'composition',
