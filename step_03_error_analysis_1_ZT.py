...
def get_error_dict(Ts_TEPZT, value_raw_on_TEPZT, value_TEP_on_TEPZT):        
    ...
    sampleid_errLnorm_dict['dZT_mean'] = np.mean(err)
    sampleid_errLnorm_dict['abs_dZT_mean'] = np.abs(sampleid_errLnorm_dict['dZT_mean'] )
    sampleid_errLnorm_dict['dZT_std_ddof0']  = np.std(err,ddof=0)
    sampleid_errLnorm_dict['dZT_std_ddof1']  = np.std(err,ddof=1)
    sampleid_errLnorm_dict['dZT_std_ddof2']  = np.std(err,ddof=2)
    sampleid_errLnorm_dict['L1']   = err_norm_calc(err,n_norm=1)
    sampleid_errLnorm_dict['L2']   = err_norm_calc(err,n_norm=2)
    sampleid_errLnorm_dict['L3']   = err_norm_calc(err,n_norm=3)
    sampleid_errLnorm_dict['L4']   = err_norm_calc(err,n_norm=4)
