...
# unique identifies for TEProp: DO NOT MODIFY
ELEC_COND = "elec_cond"
SEEBECK   = "Seebeck"
THRM_COND = "thrm_cond"
...
class SteadyState:    
    ...
    def __init__(self, device, environment, debugging=True):
        self.dev = device           # device has x-grids, material properties.
    ...
    def TE_props_at_xs(self, Ts):   # Ts is equal to T. To emphasize that T is a vector
        alpha = self.dev.seg.composition( SEEBECK, Ts )
        rho = 1/self.dev.seg.composition( ELEC_COND, Ts )
        kappa = self.dev.seg.composition( THRM_COND, Ts )
        return (alpha, rho, kappa)

    def circuit(self, alpha, rho, kappa, T):
        x = self.dev.xs      # Leg position
        A = self.dev.A       # Leg area
        L = self.dev.L       # Leg length
        deltaT = self.env.Th - self.env.Tc
        gradT = self.dev.seg.gradient(T)
    ...
    def solve_T(self,T_prev,I):
        ...
        x = self.dev.xs
        deltaT = Th - Tc
        gradT = self.dev.seg.gradient(T_prev)
        ...
        R_L = gamma * R_TE
#        print('line-172 I  ',I)
#        print('line-173 gamma   ', gamma, 'here something wrong -_-')
#        print('line-174 R_TE Vgen  ',R_TE,Vgen)
        gradT = self.dev.seg.gradient(T)
        ...
        if self.debugging: print('line-182 gamma eta dpower  ',gamma,eta,power-power2)
        ...
        #print( "ThPrime=",ThPrime)
        #print( "TcPrime=",TcPrime)
        ...
        if self.debugging: print('line-207 ZTmPrime', Zgeneral * TmPrime)
        self.report_dict['tau'] = tau 
        self.report_dict['beta'] = beta 
    ...
    def solve(self, given_no_loop=None, max_no_loop = 100, abs_tol=1e-6, given_I=None, given_R_L=None, given_gamma=GAMMA_MODE_POWER_MAX, quiet=False):
        ...
        kappa = self.dev.seg.composition( THRM_COND, T_linear )   # evaluate kapp(T)(x)
        ...
        for n in range(max_no_loop):
            if self.debugging: print(str(n)+'th loop running... (line-259)')
            self.update_history()
#            print('line-259 R_TE Vgen  ',R_TE,Vgen)
#            print('line-260 I  ',I)

##### Current Contol ######
            #I = self.current(Vgen, R_TE, K_TE, Zgeneral, tau, beta, 0, given_R_L, given_gamma)
...
#            print('line-271 ZTmPrime', ZTmPrime)
#            print('line-272 gamma   ', test_gamma)
        ...
        if quiet is False:
            if not convergent:
                print("SolverWarning: the temperature did not converge in", max_no_loop, "loops.")
            toc = time.clock()
...
if __name__ == '__main__':
    ...
    for mat in all_mat:
        mat.elec_resi.set_interp_opt(interp_opt)
        mat.Seebeck.set_interp_opt(interp_opt)
        mat.thrm_cond.set_interp_opt(interp_opt)
    ...
    devP = Device(segP, A=1/1e6)    # (seg, area)
    devN = Device(segN, A=1/1e6)    # (seg, area)
