...
def get_error_dict(Ts_TEPZT, value_raw_on_TEPZT, value_TEP_on_TEPZT):        
    ...
    relerr = dvalue_interp_on_TEPZT / value_TEP_on_TEPZT
    ...
    sampleid_errLnorm_dict['relerr_mean']  = np.mean(relerr)
    sampleid_errLnorm_dict['relerr_L1']   = err_norm_calc(relerr,n_norm=1)
    sampleid_errLnorm_dict['relerr_L2']   = err_norm_calc(relerr,n_norm=2)
    sampleid_errLnorm_dict['relerr_L3']   = err_norm_calc(relerr,n_norm=3)
    sampleid_errLnorm_dict['relerr_L4']   = err_norm_calc(relerr,n_norm=4)
    sampleid_errLnorm_dict['relerr_Linf'] = np.max( np.abs(relerr) )
    sampleid_errLnorm_dict['relerr_Max'] = np.max(relerr)
    sampleid_errLnorm_dict['relerr_Min'] = np.min(relerr)
...
df_db_err.to_csv("./data_error_analysis/"+"relZT_error_{}.csv".format(formattedDate), index=False)
df_db_err.to_csv("./data_error_analysis/"+"relZT_error.csv", index=False)
