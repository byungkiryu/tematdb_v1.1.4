...
with st.sidebar:
    ...
    if (db_mode == 'teMatDb'):
        ...
        file_tematdb_error_csv = "relZT_error.csv"
        ...
        with st.form("Data Error Filter Criteria"):
            ...
            cri_cols = ['davgZT', 'dpeakZT','relerr_L2']
            cri_vals_def = [0.10, 0.10, 0.10]
...
with tab2:     
    ...
    with st.expander("See plots:", expanded=False):   
        ...
        def draw_ZT_error_correlation(df_db_error):
            df_to_plotly = df_db_error[ df_db_error.relerr_L2 > 0].copy()
        ...
        def draw_ZT_error_correlation2(df_db_error):
            df_to_plotly = df_db_error[ df_db_error.relerr_Linf > 0].copy()
            hover_data = ['sampleid','doi','Corresponding_author_main','davgZT','dpeakZT',
                          'relerr_mean',
                          'relerr_L2','relerr_Linf',
                          'relerr_Max','relerr_Min']
        ...
        draw_ZT_error_correlation2(df2)
    ...
    with st.expander("See plots:", expanded=False):           
        ...
        draw_ZT_error_correlation2(df4)   
    st.subheader(":red[QQ analysis]")
